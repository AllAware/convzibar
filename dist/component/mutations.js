...
import { conditionValidator, objectValidator, propertiesValidator, subjectValidator, } from "./validators";
...
async function addRelationInternal(ctx, args) {
    const { tenantId, subject, relation, object, condition, properties, createdBy, enableAuditLog, onComplete, } = args;
    const graphConfig = args.graphConfig;
    ...
    if (existingRel) {
        ...
        const conditionChanged = existingCondition !== newCondition ||
            JSON.stringify(existingConditionContext) !==
                JSON.stringify(newConditionContext);
        if (conditionChanged) {
            throw new Error(`Zbar: addRelation cannot change the condition on an existing relationship ` +
    ...
    const reverseRel = graphConfig.reverseEdges?.[object.type]?.[relation]?.[subject.type];
    if (reverseRel) {
...
async function executeOnComplete(ctx, onComplete, asyncWrites) {
    if (!onComplete)
        return;
    ...
    const kind = onComplete.action === "removeRelationBatch"
        ...
        : onComplete.action === "enqueueRemoveChunkBatch"
            ? "enqueueRemoveChunk"
            : onComplete.action;
    const isBatch = onComplete.action === "removeRelationBatch" ||
        onComplete.action === "enqueueRemoveChunkBatch";
    const argsList = isBatch ? onComplete.args : [onComplete.args];
...
async function processAddChunkInternal(ctx, args) {
    const { tenantId, baseRelId, queue, graphConfig, onComplete, asyncWrites } = args;
    ...
    while (queue.length > 0 && (!asyncWrites || processed < CHUNK_SIZE)) {
        ...
        if (!eff) {
            ...
            const pathExists = eff.paths.some((p) => pathKey(p) === currentPathKey);
            if (!pathExists) {
...
async function deleteBaseRelationAndLog(ctx, args) {
    const { tenantId, subject, relation, object, actorId, enableAuditLog, existingRel, } = args;
...
async function removeRelationInternal(ctx, args) {
    const { tenantId, subject, relation, object, actorId, enableAuditLog, expectedRelId, } = args;
    const graphConfig = args.graphConfig;
    const existingRel = await ctx.db
        .query("relationships")
        .withIndex("by_tenant_subject_relation_object", (q) => q
    ...
    const inlineRemoved = await runOrEnqueue(ctx, {
        asyncWrites: args.asyncWrites,
        graphConfig,
        chunkRef: internal.mutations.processRemoveChunk,
...
async function processRemoveChunkInternal(ctx, args) {
    ...
    for (const item of queue) {
        seen.add(`${buildScopeKey(item.subject.type, item.subject.id)}:${item.relation}:${buildScopeKey(item.object.type, item.object.id)}:${item.removedRelationId}`);
    }
    while (queue.length > 0 && (!asyncWrites || processed < CHUNK_SIZE)) {
        processed++;
        const current = queue.shift();
        ...
        const eff = await ctx.db
            ...
            .unique();
        if (eff) {
            const remainingPaths = eff.paths.filter((p) => !p.baseIds || !p.baseIds.includes(current.removedRelationId));
...
export const deleteEntity = mutation({
    ...
    handler: async (ctx, args) => {
        const { tenantId, entity, actorId, graphConfig, enableAuditLog, asyncWrites, } = args;
...
export const updateRelation = mutation({
    ...
    handler: async (ctx, args) => {
        const { tenantId, subject, oldRelation, newRelation, object, condition, properties, createdBy, enableAuditLog, graphConfig, asyncWrites, } = args;
...
export const setRelation = mutation({
    ...
    handler: async (ctx, args) => {
        const { tenantId, subject, relation, object, condition, properties, createdBy, enableAuditLog, graphConfig, asyncWrites, } = args;
        ...
        const relationsToDrop = results.filter((r) => r !== null);
        ...
        const onComplete = onCompleteArgs.length > 0
            ? {
