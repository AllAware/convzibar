...
import { objectValidator, propertiesValidator, subjectValidator, } from "./validators";
...
// ---------------------------------------------------------------------------
function reverseEdgeName(config, objectType, relation, subjectType) {
    return config.reverseEdges?.[objectType]?.[relation]?.[subjectType];
}
async function findRelationship(ctx, subject, relation, object) {
    return ctx.db
        .query("relationships")
 ...
 */
async function insertReverseEdge(ctx, config, subject, relation, object) {
...
async function deleteReverseEdge(ctx, config, subject, relation, object) {
    const reverseRel = reverseEdgeName(config, object.type, relation, subject.type);
    if (!reverseRel)
        return null;
    const existing = await findRelationship(ctx, object, reverseRel, subject);
...
async function processAddChunkInternal(ctx, args) {
    ...
    while (queue.length > 0 && (!asyncWrites || processed < CHUNK_SIZE)) {
        ...
        else if (!eff.paths.some((p) => pathKey(p) === currentPathKey)) {
...
// ---------------------------------------------------------------------------
async function executeOnComplete(ctx, onComplete, config, configHash, asyncWrites) {
    if (!onComplete)
        return;
...
async function deleteBaseRelation(ctx, config, existingRel) {
...
async function processRemoveChunkInternal(ctx, args) {
    ...
    let processed = 0;
    const seen = new Set();
    const queueKey = (s, rel, o, id) => `${buildScopeKey(s.type, s.id)}:${rel}:${buildScopeKey(o.type, o.id)}:${id}`;
    ...
    while (queue.length > 0 && (!asyncWrites || processed < CHUNK_SIZE)) {
        ...
        if (!eff)
            continue;
        const remainingPaths = eff.paths.filter((p) => !p.baseIds || !p.baseIds.includes(cascadeId));
        ...
        for (const { rule, derivedSubject, derivedObject } of await collectRuleDerivations(ctx, current, config)) {
            enqueue(derivedSubject, rule.derivedRelation, derivedObject);
        }
        // Effective reverse-edge cascade (mirror of the add path).
        const reverseRel = reverseEdgeName(config, current.object.type, current.relation, current.subject.type);
        if (reverseRel) {
...
export const setRelation = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const queries = objectRelations
            .filter((r) => r !== relation)
            .map((candidateRel) => findRelationship(ctx, subject, candidateRel, object));
...
export const deleteEntity = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const remove = async (subject, relation, object) => {
        ...
        const subjectMatches = await ctx.db
            .query("relationships")
            .withIndex("by_subject_relation_object", (q) => q.eq("subjectType", entity.type).eq("subjectId", entity.id))
            .collect();
        for (const m of subjectMatches) {
            await remove({ type: m.subjectType, id: m.subjectId }, m.relation, { type: m.objectType, id: m.objectId });
        }
        const objectMatches = await ctx.db
            .query("relationships")
            .withIndex("by_object", (q) => q.eq("objectType", entity.type).eq("objectId", entity.id))
            .collect();
