...
export function getEntityRelations(z, entityType) {
 ...
 * re-parsing strings.
 */
function* iterateObjectRelationTargets(z, objectType) {
    const objectDef = z.schema.entities[objectType];
    if (!objectDef?.relations)
        return;
    const classifyCtx = {
        localRelations: objectDef.relations,
 ...
 */
export function getViaRelevantRelations(z, objectType, acceptableRelations, viaType) {
    const objectDef = z.schema.entities[objectType];
    if (!objectDef?.relations)
        return [];
    ...
    for (const rtPath of z.graphConfig.readTimePaths ?? []) {
        if (rtPath.objectType !== objectType)
            continue;
        if (!acceptableRelations.includes(rtPath.derivedRelation))
            continue;
        if (!rtPath.sourceTypes.includes(viaType))
            continue;
 ...
 */
export function getStructuralRelations(z, objectType, subjectType) {
    const result = [];
    const seen = new Set();
    for (const { relName, target } of iterateObjectRelationTargets(z, objectType)) {
        if (seen.has(relName))
            continue;
        if ((target.kind === "typed" || target.kind === "entity") &&
 ...
 */
export function getReverseStructuralRelations(z, objectType, viaType) {
    const result = [];
